...
<style>
  ...
  box-shadow: 0 3px 6px rgba(0, 0, 0, 0.18), 0 3px 6px rgba(0, 0, 0, 0.23);
    ...
    .active2{
        outline-style: solid;
        outline-color: #9162dd;
        outline-width: 2px; padding: 1%;
        border-radius: .5rem;
    }
</style>
    ...
    <form class="form" name="miFormulario" id="miFormulario" action="<?php echo base_url('/registrarventa'); ?>" method="post">
        <div class="row">
            <div class="col-lg-6  col-sm-12">
                ...
                if ($NumeroTarjetas < 3) {
                    echo '<button type="button" class="btn btn-outline-primary mt-3 mb-3 btn-modal btn-sm shadow-none" data-bs-toggle="modal" data-bs-target="#exampleModal2">Agregar Tarjeta</button>';
                } else {
                    echo '<button type="button" class="btn btn-outline-primary mt-3 mb-3 btn-modal btn-sm shadow-none" data-bs-toggle="modal" data-bs-target="#exampleModal2" disabled>Agregar Tarjeta</button>';
                }
                ...
                <div class="tarjetas-container">
                    ...
                    <?php
                        if($existe == TRUE){
                            ...
                            echo '<button type="submit" class="purchase--btn">Pagar $'.$TotalPago.'</button>';
                        }else{
...
<script>
  ...
  $('.tarjeta input[type="radio"]').change(function() {
    $('.tarjeta').removeClass('active2');
    $(this).parents('.tarjeta').addClass('active2');
  });
  ...
  $('.siguiente-btn').click(function() {
    var $active = $('.tarjeta.active2');
    var $next = $active.next('.tarjeta');
    if($next.length) {
